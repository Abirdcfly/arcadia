...
func cr2model(objPrompt, objChain, objRetriever, objApp *unstructured.Unstructured) (*generated.KnowledgeBaseApplication, error) {
	...
	UpdateTimestamp := &app.CreationTimestamp.Time
	status := "Pending"
	if conditions := app.Status.Conditions; len(conditions) > 0 {
		UpdateTimestamp = &conditions[0].LastTransitionTime.Time
		status = string(conditions[0].Type)
	}
...
func CreateKnowledgeBaseApplication(ctx context.Context, c dynamic.Interface, input generated.CreateKnowledgeBaseApplicationInput) (*generated.KnowledgeBaseApplication, error) {
	...
	objPrompt, err := c.Resource(promptSchem).Namespace(input.Namespace).Create(ctx, &unstructured.Unstructured{Object: object}, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	...
	objChain, err := c.Resource(chainSchem).Namespace(input.Namespace).Create(ctx, &unstructured.Unstructured{Object: object}, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	...
	objRetriever, err := c.Resource(retrieverSchem).Namespace(input.Namespace).Create(ctx, &unstructured.Unstructured{Object: object}, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	...
	objApp, err := c.Resource(appSchem).Namespace(input.Namespace).Create(ctx, &unstructured.Unstructured{Object: object}, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
...
func UpdateKnowledgeBaseApplication(ctx context.Context, c dynamic.Interface, input generated.UpdateKnowledgeBaseApplicationInput) (*generated.KnowledgeBaseApplication, error) {
	// 1. get old resource
	objPrompt, err := c.Resource(promptSchem).Namespace(input.Namespace).Get(ctx, input.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	objChain, err := c.Resource(chainSchem).Namespace(input.Namespace).Get(ctx, input.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	objRetriever, err := c.Resource(retrieverSchem).Namespace(input.Namespace).Get(ctx, input.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	objApp, err := c.Resource(appSchem).Namespace(input.Namespace).Get(ctx, input.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	...
	if (input.SystemMessage != nil && *input.SystemMessage != prompt.Spec.SystemMessage) || (input.UserMessage != nil && *input.UserMessage != prompt.Spec.UserMessage) {
		...
		objPrompt, err = c.Resource(promptSchem).Namespace(input.Namespace).Update(ctx, &unstructured.Unstructured{Object: object}, metav1.UpdateOptions{})
		if err != nil {
			return nil, err
		}
	}
	...
	if input.KnowledgebaseName != "" && input.KnowledgebaseName != retriever.Spec.Input.KnowledgeBaseRef.Name {
		...
		objRetriever, err = c.Resource(retrieverSchem).Namespace(input.Namespace).Update(ctx, &unstructured.Unstructured{Object: object}, metav1.UpdateOptions{})
		if err != nil {
			return nil, err
		}
	}
	...
	if !reflect.DeepEqual(app, oldApp) {
		...
		objApp, err = c.Resource(appSchem).Namespace(input.Namespace).Update(ctx, &unstructured.Unstructured{Object: object}, metav1.UpdateOptions{})
		if err != nil {
			return nil, err
		}
	}
...
func DeleteKnowledgeBaseApplication(ctx context.Context, c dynamic.Interface, input generated.DeleteCommonInput) (*string, error) {
	resources := []dynamic.NamespaceableResourceInterface{
		c.Resource(promptSchem),
		c.Resource(chainSchem),
		c.Resource(retrieverSchem),
		c.Resource(appSchem),
	}
...
func ListKnowledgeBaseApplications(ctx context.Context, c dynamic.Interface, input generated.ListCommonInput) (*generated.PaginatedResult, error) {
	...
	res, err := c.Resource(appSchem).Namespace(input.Namespace).List(ctx, metav1.ListOptions{
