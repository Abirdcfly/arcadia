...
func GetDefaultRerankEndpoint(ctx context.Context, c client.Client) (string, error) {
	config, err := GetConfig(ctx, c)
	if err != nil {
		return "", err
	}
	if endpoint := config.RerankDefaultEndpoint; endpoint == "" {
		return "", ErrNoConfigRerank
	} else {
		return endpoint, nil
	}
}
