...
func (l *KnowledgeBaseRetriever) Run(ctx context.Context, cli client.Client, args map[string]any) (map[string]any, error) {
	...
	if len(docs) == 0 && instance.Spec.DocNullReturn != "" {
		val, ok := args[base.InputIsNeedStreamKeyInArg]
		if ok {
			needStream, _ := val.(bool)
			if needStream {
				streamChan, ok := args[base.OutputAnserStreamChanKeyInArg].(chan string)
				if ok && streamChan != nil {
					go func() {
						streamChan <- instance.Spec.DocNullReturn
					}()
				}
			}
		}
		return nil, &base.AppStopEarlyError{Msg: instance.Spec.DocNullReturn}
	}
